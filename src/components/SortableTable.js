...
const TextBoxes = (props) => {
    ...
    let count = [];
    for (let i = 0; i < length; i += 1) count.push(i);
    ...
    useEffect(() => {
        if (filteredStats) {
            filteredStats.sort((a, b) => {
                if (!a[sortStat] && !b[sortStat]) return 0;
                else if (!a[sortStat]) return dir === 'descending' ? 1 : -1;
                else if (!b[sortStat]) return dir === 'descending' ? -1 : 1;

                // Ensure that the stat is a string before calling .replace()
                const aStat = a[sortStat] ? a[sortStat].toString().replace(/,/g, '') : '';
                const bStat = b[sortStat] ? b[sortStat].toString().replace(/,/g, '') : '';
                ...
                <thead>
                    <tr>
                        {Object.keys(indeces).map((key, i) => {
                            return <th key={`th-key-${key}`} className='sort-header' onClick={() => changeSort(key)}>
                                {indeces[key]} {sortStat === key && <img src={dir === 'ascending' ? ascendingIcon : descendingIcon} height='15' alt='sort' />}
                            </th>;
                        })}
                    </tr>
